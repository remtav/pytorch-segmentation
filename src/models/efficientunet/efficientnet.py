        ...
        class Encoder(nn.Module):
            ...
            def forward(self, x):
                ...
                for idx, block in enumerate(self.blocks):
                    ...
                    x = block(x, drop_connect_rate)
                    if idx == 10:
                        block10 = x.clone().detach().requires_grad_(True)
                    elif idx == 6:
                        block6 = x.clone().detach().requires_grad_(True)
                    elif idx == 2:
                        block2 = x.clone().detach().requires_grad_(True)
                    elif idx == 0:
                        block0 = x.clone().detach().requires_grad_(True)

                # Head
                x = self.head_conv(x)
                x = self.head_batch_norm(x)
                x = self.head_swish(x)
                return block10, block6, block2, block0, x
