...
class SherbrookeDataset(Dataset):
    def __init__(self, base_dir='../data/sherbrooke', split='train',
        ...
        if defects is False:
            ...
            self.n_classes = 2
            self.void_classes = [0,4] #why 4? still some 4 values in labels?
    ...
    def __getitem__(self, index):
        ...
        lbl_sidewalks = self.encode_mask(lbl.copy(), orig=(0,4,8,34,35,36,255), dest=(0,255,1,1,1,1,255)) #overwrite values for to get a 2 class task (sidewalk/background or defect/background)
        lbl_defects = self.encode_mask(lbl.copy(), orig=(0, 4, 8, 34, 35, 36, 255), dest=(255, 255, 0, 1, 1, 1, 255))
        lbl_maj_defects = self.encode_mask(lbl.copy(), orig=(0, 4, 8, 34, 35, 36, 255), dest=(255, 255, 0, 0, 1, 1, 255))

        # if self.resizer:
        #     resized = self.resizer(image=img, mask1=lbl_sidewalks, mask2=lbl_defects, mask3=lbl_maj_defects)
        #     img, lbl_sidewalks, lbl_defects, lbl_maj_defects = resized['image'], resized['mask1'], resized['mask2'], resized['mask3']
        # # AffineAugment (Horizontal Flip, Rotate...)
        # if self.affine_augmenter:
        #     augmented = self.affine_augmenter(image=img, mask1=lbl_sidewalks, mask2=lbl_defects, mask3=lbl_maj_defects)
        #     img, lbl_sidewalks, lbl_defects, lbl_maj_defects = augmented['image'], augmented['mask1'], augmented['mask2'], augmented['mask3']

        if self.debug:
            print(f'Unique values in labels: \nSidewalk only: {np.unique(lbl_sidewalks)}\n'
                  f'Sidewalk all defects: {np.unique(lbl_defects)}'
                  f'Sidewalk major defects only: {np.unique(lbl_maj_defects)}'
                  f'Label shape: {lbl.shape}')
        else:
            img = img.transpose(2, 0, 1)
            img = torch.FloatTensor(img)
            lbl = torch.LongTensor(lbl)

        return img, lbl_sidewalks, lbl_defects, lbl_maj_defects

    def encode_mask(self, lbl, orig=(0,1,2,3,4,5), dest=(0,1,2,3,4,5)):
        ...
        assert len(orig) == len(dest)

        for orig_val, dest_val in zip(orig, dest):
            if orig_val == dest_val:
                continue
            else:
                lbl[lbl == orig_val] = dest_val
